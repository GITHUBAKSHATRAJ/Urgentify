...
import styles from './TaskDetailEditStyles';
...
// This component manages editing tasks. It pre-fills state via useEffect based on the `task` prop.
export default function TaskDetailEdit({ visible, theme, task, onSubmit, onCancel }) {
  ...
  // Pre-fill state when editing a task
  React.useEffect(() => {
    if (task && visible) {
      ...
      setFormStartDate(formatDateTime(task.startDate));
      setFormDeadline(formatDateTime(task.deadline));
      setFormSeverity(task.severity || 'low');
      
      if (task.customNotificationValue && task.customNotificationUnit !== 'none') {
        setHasCustomAlert(true);
        setFormNotificationValue(String(task.customNotificationValue));
        setFormNotificationUnit(task.customNotificationUnit);
      } else {
